...
clean_docker_images() {
  for repo in $(docker images --format "{{.Repository}}" | sort -u); do
    docker images $repo --format "{{.ID}}" | tail -n +2 | xargs docker rmi
  done
}
...
find_ports_for_pattern() {
    ...
    local pids=$(pgrep -f "$pattern" | xargs echo | sed 's/ /|/g')
    if [[ -z "$pids" ]]; then
        echo "No processes found for pa/ttern '$pattern'."
        return 1
    fi

    # Search for these PIDs in ss output
    ss -ltnp | grep -E "($pids)"
}


function fcontent() {
  local dir=${1:-.}  # Default to current directory if no directory is provided
  local pattern=$2

  # Check if the search pattern is provided
  if [ -z "$pattern" ]; then
    echo "Usage: fcontent <directory> <pattern>"
    return 1
  fi
